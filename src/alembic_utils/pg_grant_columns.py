...
@dataclass
class PGGrantColumns(ReplaceableEntity):
    """A PostgreSQL Grant Statement compatible with `alembic revision --autogenerate`

    PGGrantColumns requires the role name being used to generate migrations to
    match the role name that executes migrations.

    If your system does not meet that requirement, disable them by excluding PGGrantColumns
    ...
    * **with_grant_option** - *bool*: Can the role grant access to other roles
    ...
    @classmethod
    def from_sql(cls, sql: str) -> "PGGrantColumns":
